    ...
    (if (plusp base)
        (progn
          ...
          (when (or (> (+ base c) +trie-index-max+)
            ...
            (let* ((cs (da-collect-c da s c))
                   (new-base (da-find-free-base da cs)))
...
(defun da-collect-c (da s c)
  "base[s] を遷移元とする c を集める。"
  (let* ((base (da-get-base da s))
         (cs (loop for i from base to (max-check-state-at-base da base)
                        if (= (da-get-check da i) s)
                          collect (- i base))))
    (sort (if c (cons c cs) cs)
          #'<=)))
...
(defun da-find-free-base (da cs)
  (let* ((free-list (da-get-free-list da))
         (first-sym (car cs))
         (s (- (da-get-check da free-list))))
    (loop while (and (/= s free-list)
                     (< s (+ first-sym +da-pool-begin+)))
          do (setf s (- (da-get-check da s))))
    (when (= s free-list)
      (setf s (+ first-sym +da-pool-begin+))
      (loop do (da-extend-pool da s)
            until (minusp (da-get-check da s))
            do (incf s)))
    (loop until (da-fit-cs da (- s first-sym) cs)
          if (= (- (da-get-check da s)) free-list)
            do (da-extend-pool da (da-size da))
          do (setf s (- (da-get-check da s))))
    (- s first-sym)))

(defun da-fit-cs (da base cs)
  (loop for sym in cs
        always (and (da-check-free-cell da (+ base sym))
                    (<= base (- +trie-index-max+ sym)))))

(defun da-relocate-base (da s new-base)
  (loop with old-base = (da-get-base da s)
        for sym in (da-collect-c da s nil)
...
(defun da-enumerate-recursive (da state fun user-data)
  (let ((base (da-get-base da state)))
    (if (< base 0)
        ...
        (loop for sym in (da-collect-c da state nil)
